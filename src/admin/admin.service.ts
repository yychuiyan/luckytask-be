...
export class AdminService {
  ...
  async updateUser(
    adminUserId: number,
    targetUserId: number,
    data: { role?: UserRole; nickname?: string; username?: string },
  ) {
    await this.ensureAdmin(adminUserId);
  ...
  async getMenuConfig(role: string) {
    ...
    // 管理员系统设置始终开启
    if (role === (UserRole.ADMIN as string) && config.settings) {
  ...
  async getAllMenuConfigs(adminUserId: number) {
    ...
    const result: Record<string, MenuConfig> = {};
  ...
  async saveMenuConfig(adminUserId: number, role: string, config: MenuConfig) {
    await this.ensureAdmin(adminUserId);
